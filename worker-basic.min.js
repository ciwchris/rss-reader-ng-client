...
function fetchFromCacheInstruction(worker, req, cache) {
    var op = function () { return worker.cache.load(cache, req); };
...
function rewriteUrlInstruction(worker, req, destUrl) {
    ...
    var op = function () { return worker.fetch(newReq); };
    op.desc = { type: 'rewriteUrlInstruction', worker: worker, req: req, destUrl: destUrl };
    return op;
}

var VersionWorkerImpl = (function () {
    function VersionWorkerImpl(streamController, scope, manifest, adapter, cache, fetcher, plugins) {
    ...
    VersionWorkerImpl.prototype.fetch = function (req) {
        ...
        return instructions.reduce(function (prev, curr) { return prev.then(function (resp) { return resp ? resp : curr(); }); }, Promise.resolve(null));
    };
...
var hashes_1 = hashes.SHA1;

function parseManifest(data) {
    var manifest = JSON.parse(data);
    manifest._json = data;
    manifest._hash = new hashes_1().hex(data);
    return manifest;
}
...
var Driver = (function () {
            ...
            event.respondWith(_this
                ...
                switch (_this.state) {
                    ...
                    case DriverState.LAME:
                        ...
                        return _this.fetcher.request(req);
                    default:
    ...
    Driver.prototype.workerFromManifest = function (manifest) {
        var plugins = [];
        var worker = new VersionWorkerImpl(this, this.scope, manifest, this.adapter, new ScopedCache(this.scopedCache, "manifest:" + manifest._hash + ":"), this.fetcher, plugins);
...
var RouteRedirectionImpl = (function () {
    ...
    RouteRedirectionImpl.prototype.fetch = function (req, ops) {
        ...
        var matchesRoutingTable = Object.keys(manifest.routes).some(function (route) {
            ...
            var matchesPath = config.prefix
                ? path.indexOf(route) === 0
                : path === route;
            var matchesPathAndExtension = matchesPath &&
                (!config.onlyWithoutExtension || !_this.hasExtension(path));
